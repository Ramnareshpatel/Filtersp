...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        ...
        btn.append(
            [InlineKeyboardButton(f"📜 ᴘᴀɢᴇ {round(int(offset) / 10) + 1} / {round(total / 6)}", callback_data="pages"),
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('ʏᴛ-sᴇᴀʀᴄʜ', callback_data='ytsearch'),
        ...
        buttons = [[
            ...
            InlineKeyboardButton('ᴡɪᴋɪᴘᴇᴅɪᴀ', callback_data='wiki'),
            InlineKeyboardButton('ɪᴘ-ᴀᴅᴅʀᴇss', callback_data='ipaddress'),
        ...
        buttons = [[
            InlineKeyboardButton('😎 Dᴇᴠᴇʟᴏᴘᴇʀs', callback_data='dev')
            ],[
            InlineKeyboardButton('🏠 Hᴏᴍᴇ', callback_data='start'),
        ...
        buttons = [[
            InlineKeyboardButton('« Bᴀᴄᴋ', callback_data='prop'),
            InlineKeyboardButton('Lᴀɴɢ Cᴏᴅᴇs', url='https://cloud.google.com/translate/docs/languages')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        if str(grp_id) != str(grpid):
            await query.message.edit("<i><b>Yᴏᴜʀ Aᴄᴛɪᴠᴇ Cᴏɴɴᴇᴄᴛɪᴏɴs Hᴀs Bᴇᴇɴ Cʜᴀɴɢᴇᴅ Gᴏ Tᴏ./settings</b></i>")
            return 
...
async def advantage_spell_chok(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply("<i><b>😓 I Cᴏᴜʟᴅɴ'ᴛ Fɪɴᴅ Aɴʏ Mᴏᴠɪᴇ Iɴ Tʜᴀᴛ Nᴀᴍᴇ.</b></i>")
